...
export class ISelect implements AfterViewInit {
		...
		if (key === 40) {
			...
			if (this.searchInput) {
				setTimeout(() => {
					...
					this.searchInput.element.nativeElement.select();
				}, 22);
			}
		} else if (key === 38 && this.highlightIndex === 0) {
			this.config.open = false;
			this.searchButton.element.nativeElement.focus();
		}
		return false;
	}
	keyup(event: any) {
	...
	performSearch(event: any, searchString:string): any {
		let key = event.charCode || event.keyCode || 0;
		if (key > 36 && key < 41) {
	...
	resetSearch() {
		this.renderer.setAttribute(this.searchInput.element.nativeElement, 'value', '');
